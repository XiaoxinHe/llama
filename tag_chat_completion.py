...
def main(
    ...
    for i in tqdm(range(0, len(dialogs), max_batch_size)):
        ...
        for uid, dialog, result in zip(batch_uids, batch_dialogs, results):
            result['prompt'] = dialog
            with open(f'output/{dataset}/{uid}.json', 'w')as f:
